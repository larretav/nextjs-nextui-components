    ...
    <div className="w-full grid grid-cols-1 sm:grid-cols-4 gap-4">
      {
        categories.map((item, idx) => (
          <Card isPressable onPress={()=> router.push(item.url)} className=" p-4 rounded-2xl  bg-zinc-100 dark:bg-zinc-800 text-slate-800 dark:text-slate-100 gap-3 items-center">
            <div>{item.icon}</div>
